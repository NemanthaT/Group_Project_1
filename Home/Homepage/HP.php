<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>EDSA Lanka Consultancy - About Us</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
    <link rel="stylesheet" href="HP.css">
</head>
<body>
    <div class="top">
        <!-- Navbar -->
        <nav class="navbar" id="nbar">
            <div class="Hcontainer">
                <div class="header_logo">
                        <img src="img/logo.png" alt="EDSA Logo">
                </div>
                <ul class="header_item">
                        <li><a href="../Homepage/homePage.php" target="Homepage">Home</a></li>
                        <li><a href="../Our_Services/OS.php" target="Homepage">Our Services</a></li>
                        <li><a href="../News/News.php" target="Homepage">News</a></li>
                        <li><a href="../Knowledge_Base/KB.php" target="Homepage">Knowledge Base</a></li>
                        <li><a href="../About_Us/AU.php" target="Homepage">About Us</a></li>
                        <!--<li><a href="#">Community</a></li>-->
                        <li><a href="../Contact_Us/CU.php" target="Homepage">Contact Us</a></li>
                </ul>
                <a href="../../Login/login.php" class="logout">login</a>
            </div>
        </nav>
    </div>
    <div id=frame>
        <iframe src="homePage.php" name="Homepage" title="Homepage"></iframe>
    </div>
<!--<main>-->
    <!-- Hero Section -->
    <!--<section class="hero">
        <div class="hero-content">
            <h1>EDSA Lanka Consultancy</h1>
            <p>A premier partner for education, training, consulting, and research</p>
            <div class="hero-image">
                <img src="#" alt="Hero Image">
            </div>
            <div class="hero-indicators">
                <span class="indicator"></span>
                <span class="indicator"></span>
                <span class="indicator"></span>
            </div>
        </div>
    </section>-->

    <!-- Our Clients Section -->
    <!--<section class="clients">
        <h2>Our Clients</h2>
        <div class="client-logos">
            <div class="client-logo"><img src="#" alt="Client 1"></div>
            <div class="client-logo"><img src="#" alt="Client 2"></div>
            <div class="client-logo"><img src="#" alt="Client 3"></div>
        </div>
    </section>-->

    <!-- News Section -->
    <!--<section class="news">
        <h2>News</h2>
        <div class="news-content">
            <div class="news-image">
                <img src="#" alt="News">
            </div>
            <div class="news-text">
                <h3>EDSA Lanka Consultancy Rebrands as Lanka Edu Solutions</h3>
                <p>
                    July 7, 2024 – Colombo, Sri Lanka – EDSA Lanka Consultancy is excited to announce its rebranding as Lanka Edu Solutions. This transition marks a renewed emphasis on innovation in education, training, and consulting services, strengthening its mission to address the growing challenges faced by organizations and research across Sri Lanka and beyond.
                </p>
            </div>
        </div>
    </section>
</main>-->

<div>
 <!-- Footer -->
 <footer>
    <div class="Fcontainer">
        <!-- Quick Links -->
        <div class="Fquick-links">
            <h3>Quick Links</h3>
            <ul>
                <li><a href="../Homepage/homePage.php" target="Homepage">Home</a></li>
                <li><a href="../Our_Services/OS.php" target="Homepage">Our Services</a></li>
                <li><a href="../News/News.php" target="Homepage">News</a></li>
                <li><a href="../About_Us/AU.php" target="Homepage">About Us</a></li>
                <li><a href="../Knowledge_Base/KB.php" target="Homepage">Knowledge Base</a></li>
                <li><a href="../Contact_Us/CU.php" target="Homepage">Contact Us</a></li>
                <!--<li><a href="#">Community</a></li>-->
            </ul>
        </div>
        <!-- Contact Info -->
        <div class="Fcontact-info">
            <h3>Contact Us</h3>
            <ul>
                <li>Email: info@edsalanka.com</li>
                <li>Phone: +94 123 456 789</li>
                <li>Address: 123 EDSA Lane, Colombo, Sri Lanka</li>
            </ul>
            <!-- Social Media Links -->
            <div class="Fsocial-media">
                <!--<a href="#"><img src="../../images/facebook.jpg" alt="Facebook"></a>-->
                <a href="#" class="fa fa-facebook"></a>
                <a href="#" class="fa fa-linkedin"></a>
                <a href="#" class="fa fa-instagram"></a>
            </div>
        </div>
        <!-- Footer Logo -->
        <div class="Flogo">
            <img src="img/logo.png" alt="EDSA Lanka Consultancy Logo">
        </div>
    </div>
    <div class="Fcopyright">
        &copy; 2024 EDSA Lanka Consultancy. All rights reserved.
    </div>
</footer>
</div>
</body>
</html>
